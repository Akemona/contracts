...
abstract contract LinearTimelock {
  struct InvestorParam {
    IAkenro.InvestorType investorType;
  ...
  function _proportionAvailable(
    ...
    if (investorParam.investorType == IAkenro.InvestorType.SeedTranche) {
      ...
      uint96 vestedAmount =
        safe96(
          ...
          "Akenro::_proportionAvailable: amount exceeds 96 bits"
        );
      return
        add96(
          vestedAmount,
          investorParam.initialBalance / 3,
          "Akenro::_proportionAvailable: overflow"
        );
    } else {
      return
        safe96(
...
contract Akenro is AccessControl, LinearTimelock {
  /// @notice EIP-20 token name for this token
  string public constant name = "Akenro";

  /// @notice EIP-20 token symbol for this token
  string public constant symbol = "AKENRO";
  ...
  uint256 public totalSupply = 1_000_000_000e18; // 1 billion Akenro

  // @modified used in revokeTranch to return tokens to governance address
  ...
  modifier whenTransferAllowed() {
    require(
      ...
      "AkenroToken: Transfers not allowed or not right privillege"
    );
    _;
  }

  /**
   * @notice Construct a new Akenro token
    ...
    )
  {
    ...
    // @modified no minting after deployment
  ...
  function approve(address spender, uint256 rawAmount) external returns (bool) {
    ...
    if (rawAmount == type(uint256).max) {
      ...
      amount = safe96(rawAmount, "Akenro::approve: amount exceeds 96 bits");
    }
  ...
  function transfer(address dst, uint256 rawAmount) external returns (bool) {
    uint96 amount = safe96(rawAmount, "Akenro::transfer: amount exceeds 96 bits");
  ...
  function transferFrom(
    ...
    uint96 amount = safe96(rawAmount, "Akenro::approve: amount exceeds 96 bits");
    ...
    if (spender != src && spenderAllowance != type(uint96).max) {
      uint96 newAllowance =
        sub96(
          spenderAllowance,
          amount,
          "Akenro::transferFrom: transfer amount exceeds spender allowance"
        );
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "Akenro::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "Akenro::delegateBySig: invalid nonce");
    require(
      block.timestamp <= expiry,
      "Akenro::delegateBySig: signature expired"
    );
    ...
    returns (uint96)
  {
    require(
      ...
      "Akenro::getPriorVotes: not yet determined"
    );
  ...
  function mint(address account, uint256 rawAmount) external {
    require(hasRole(MINTER_ROLE, msg.sender), "Akenro::mint: not authorized");
    require(account != address(0), "cannot mint to the zero address");

    uint96 amount = safe96(rawAmount, "Akenro::mint: amount exceeds 96 bits");
    uint96 supply =
      safe96(totalSupply, "Akenro::mint: totalSupply exceeds 96 bits");
    totalSupply = add96(supply, amount, "Akenro::mint: token supply overflow");
    balances[account] = add96(
      balances[account],
      amount,
      "Akenro::mint: balance overflow"
    );
  ...
  function _transferTokens(
    ...
    require(
      src != address(0),
      "Akenro::_transferTokens: cannot transfer from the zero address"
    );
    require(
      dst != address(0),
      "Akenro::_transferTokens: cannot transfer to the zero address"
    );
    if (investorBalances[src].initialBalance > 0) {
      require(
        amount <= _getFreedBalance(src),
        "Akenro::_transferTokens: not enough unlocked balance"
      );
    }
    ...
    balances[src] = sub96(
      balances[src],
      amount,
      "Akenro::_transferTokens: transfer amount exceeds balance"
    );
    balances[dst] = add96(
      balances[dst],
      amount,
      "Akenro::_transferTokens: transfer amount overflows"
    );
  ...
  function _moveDelegates(
    ...
    if (srcRep != dstRep && amount > 0) {
      if (srcRep != address(0)) {
        ...
        uint96 srcRepNew =
          sub96(srcRepOld, amount, "Akenro::_moveVotes: vote amount underflows");
      ...
      if (dstRep != address(0)) {
        ...
        uint96 dstRepNew =
          add96(dstRepOld, amount, "Akenro::_moveVotes: vote amount overflows");
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber =
      safe32(
        block.number,
        "Akenro::_writeCheckpoint: block number exceeds 32 bits"
      );
  ...
  function enableTransfer() external {
    require(
      ...
      "Akenro::enableTransfer: not authorized"
    );
  ...
  function updateTrancheBalance(
    ...
    require(rawAmount > 0, "Akenro::updateTrancheBalance: amount must be > 0");
    require(
      investorBalances[beneficiary].initialBalance == 0,
      "Akenro::updateTrancheBalance: already has timelocked Akenro"
    ); //Prevents users from being in more than 1 tranche

    uint96 amount =
      safe96(rawAmount, "Akenro::updateTrancheBalance: amount exceeds 96 bits");
    investorBalances[beneficiary] = InvestorParam(investorType, amount);
  }

  function revokeTranche(address account) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Akenro::revokeTranche: not authorized");
    require(
      investorBalances[account].initialBalance > 0,
      "Akenro::updateTrancheBalance: account has no timelocked Akenro"
    );
    require(
      passedAllVestingPeriods() == false,
      "Akenro::updateTrancheBalance: account has no timelocked Akenro"
    );
    ...
    if (passedCliff()) {
      // Return locked balance to governance
      (uint256 vestingPeriod, uint256 elapsed) =
            _getTrancheInfo(investorParam.investorType);
      uint96 lockedBalance =
            sub96(
              ...
              "Akenro::getFreedBalance: locked balance underflow"
            );

      investorParam.initialBalance = 0;
      balances[account] = sub96(balances[account], lockedBalance, "Akenro::revokeTranche: balance underflow");
      balances[governanceAddress] = add96(balances[governanceAddress], lockedBalance, "Akenro:revokeTranche: balance overflow");
    } else {
      //we have not hit the cliff yet, all investor balance is locked
      uint96 initialBalance = investorParam.initialBalance;

      balances[account] = sub96(balances[account], initialBalance, "Akenro::revokeTranche: balance underflow");
      balances[governanceAddress] = add96(balances[governanceAddress], initialBalance, "Akenro:revokeTranche: balance overflow");
    }
  ...
  /**
   * @notice Internal function the amount of unlocked Akenro for an account that participated in Public/Seed Investments
   */
      ...
      if (passedCliff()) {
        ...
        return
          sub96(
            balances[account],
            lockedBalance,
            "Akenro::getFreedBalance: total freed balance underflow"
          );
      } else {
        //we have not hit the cliff yet, all investor balance is locked
        return
          sub96(
            balances[account],
            investorParam.initialBalance,
            "Akenro::getFreedBalance: balance underflow"
          );
      }
    }
  }
  ...
  /*
  @Modified do not allow cliff timestamp to be updated after deployment
  function updateCliffTimestamp(uint256 newTimestamp) external {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
      "Akenro::updateCliffTimestamp: not authorized"
    );
