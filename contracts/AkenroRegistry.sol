...
import "./AkenroDistributor/openzeppelin/contracts/access/AccessControl.sol";
import "./AkenroDistributor/openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "./AkenroDistributor/contracts/interfaces/ITrancheToken.sol";
import "./AkenroDistributor/contracts/interfaces/IRegistry.sol";
import "./AkenroDistributor/contracts/libraries/AkenroLibrary.sol";
import "./AkenroDistributor/contracts/interfaces/IWETH.sol";

/**
 * @title Global values used by many contracts
 * @notice This is mostly used for access control
 */
contract AkenroRegistry is IRegistry, AccessControl {
  using EnumerableSet for EnumerableSet.AddressSet;
  bool private _paused;
  bool public tokenMinting;/* @modified: removed override */
  ...
  constructor(
    ...
    _setupRole(AkenroLib.GOVERNANCE_ROLE, _governance);
    _setRoleAdmin(AkenroLib.VAULT_ROLE, AkenroLib.DEPLOYER_ROLE);
    _setRoleAdmin(AkenroLib.ROLLOVER_ROLE, AkenroLib.DEPLOYER_ROLE);
    _setRoleAdmin(AkenroLib.STRATEGY_ROLE, AkenroLib.DEPLOYER_ROLE);
  ...
  function addStrategist(address _strategist, string calldata _name) external {
    grantRole(AkenroLib.STRATEGIST_ROLE, _strategist);
    strategistNames[_strategist] = _name;
  }

  /* @modified removed override */
  function enableTokens() external onlyRole(AkenroLib.GOVERNANCE_ROLE) {
    tokenMinting = true;
  }

  /* @modified removed override */
  function disableTokens() external onlyRole(AkenroLib.GOVERNANCE_ROLE) {
    tokenMinting = false;
  }
   ...
   */
  function pause() external override onlyRole(AkenroLib.PANIC_ROLE) {
    _paused = true;
   ...
   */
  function unpause() external override onlyRole(AkenroLib.GUARDIAN_ROLE) {
    _paused = false;
  ...
  function tokensDeclaredDead(address[] calldata _tokens)
    external
    onlyRole(AkenroLib.GUARDIAN_ROLE)
  {
  ...
  function setFallbackRecipient(address payable _target)
    external
    onlyRole(AkenroLib.GOVERNANCE_ROLE)
  {
